...
const GameDetails = () => {
  ...
  const handleParticipate = async () => {
    // Confirm athleteId and user availability before proceeding
    if (!athleteId) {
      setError("Error: Athlete ID not found. Please log in again.");
      return;
    }

    try {
      // Attempt to add the athlete to the game
      await addAthleteToGame(gameId, athleteId);

      // Set success status
      setParticipationStatus("Successfully added athlete to game.");

      // Refresh game details to reflect participation change
      const updatedGame = await getGameById(gameId);
      setGame(updatedGame);
    } catch (err) {
      // Log specific error and set a more descriptive error message
      console.error("Error in participate action:", err.message);
      setError(`Failed to participate in game: ${err.message}`);
    }
  };
